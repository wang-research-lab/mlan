...
def prompt_llama_plain(input, model_specific_prompt_kwargs=None):
    ...
    if choices_formatted:
        return f'<image>\nQuestion: {question} Choose the best option from below:\n{choices_formatted}\nAnswer: The best answer is "'
    elif "yes" in input['answer'] or "no" in input['answer']:
        return f'<image>\nQuestion: {question} Please answer the question with "yes" or "no".\nAnswer: The best answer is "'
    else: 
        return f'<image>\nQuestion: Please provide a short answer to the question: {question}\nAnswer: The best answer is "'
